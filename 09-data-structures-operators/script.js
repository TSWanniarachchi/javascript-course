...
console.log(`Average odd: ${sumOfOdds / odds.length}`);

// 3.
for (const [team, score] of Object.entries(game.odds)) {
  game[team]
    ? console.log(`Odd of victory ${game[team]}: ${score}`)
...
console.log(new Set("Tharindu Sachin Wanniarachchi").size);
*/
...
console.log(new String("Sachin"));
console.log(typeof new String("Sachin"));
console.log(typeof new String("Sachin").slice(1));
*/
...
console.log("Sachin".toUpperCase());

// Fix capitalization in name
const passenger = "sAchIN"; // Sachin
...
// Comparing emails
const email = "hello@sachin.io";
const loginEmail = "  Hello@Sachin.Io \n";
...
console.log("Sachin Wanniarchchi".split(" "));
const [firstName, lastName] = "Sachin Wanniarchchi".split(" ");
...
capitalizeName("thariNdu sachin waNniarachchi");

// Padding
const message = "Go to gate 23!";
console.log(message.padStart(20, "+").padEnd(30, "+"));
console.log("Sachin".padStart(20, "+").padEnd(30, "+"));
...
/*
Write a program that receives a list of variable names written in underscore_case and convert them to camelCase. The input will come from a textarea inserted into the DOM (see code below), and conversion will happen when the button is pressed.
...
document.querySelector("button").addEventListener("click", function () {
  ...
  const variableNames = textAreaContent.split("\n");

  for (const [variableNameIndex, variableName] of variableNames.entries()) {
    const words = variableName.toLowerCase().trim().split("_");
    const camelCaseWords = [];

    for (const [wordIndex, word] of words.entries()) {
      if (wordIndex === 0) camelCaseWords.push(word);
      else camelCaseWords.push(word.replace(word[0], word[0].toUpperCase()));
    }
    const camelCaseVariableName = camelCaseWords.join("");
    const checkMarks = "✅".repeat(variableNameIndex + 1);

    console.log(`${camelCaseVariableName.padEnd(20)}${checkMarks}`);
  }
});
*/
...
for (const flight of flights.split("+")) {
  ...
  const outputStr = `${type.includes("Delayed") ? "🔴" : ""} ${type
    .slice(1)
